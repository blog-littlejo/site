...
Vous allez voir que cela peut être très intéressant pour du profilage, du *debug* ou de la rétro-ingénierie.
...
La kRetProbe est simplement dédiée à l'observation du retour des fonctions du kernel Linux.

Nous avons vu brièvement kProbe et kRetProbe qui pourraient faire l'objet d'un autre article. Parlons maintenant des probes qui nous intéressent : uProbe et uRetProbe.

### uProbe

Contrairement aux **k**Probes qui sont dédiées à observer les fonctions du **k**ernel Linux, les **u**Probes sont dédiées aux fonctions de l'espace utilisateur : *User Probes*. Par exemple, on pourrait s'en servir pour compter le nombre d'appels aux fonctions `malloc` et `free` dans un programme C. uProbe permet également de récupérer le contenu des arguments de la fonction observée.

Pour finir, uProbe peut être utilisé pour débugger et comprendre un programme dont tu n'as pas le code source. Ça peut donc être un bel outil de rétro-ingénierie (*reverse engineering*).
...
On peut donc découvrir la valeur que retourne la fonction. Cela permet ainsi de debugger ou d'observer le comportement final de la fonction.

Mais il y a un autre intérêt : en combinant les temps de l'uProbe et de l'uRetProbe, on peut récupérer la durée que met une fonction à s'exécuter assez facilement. Il est ainsi possible de profiler chaque fonction de son programme.
...
Les uRetProbes et uProbes ne peuvent s'utiliser que pour les programmes dont le langage est compilé : C/C++, Rust, Go etc. Si on a un programme développé avec un autre langage, USDT pourrait vous convenir.

### USDT

USDT veut dire *User Statically-Defined Tracing*. Elle est dérivée de l'uprobe.
Comme la uProbe, elle est dédiée aux programmes de l'espace utilisateur mais il faut rajouter dans le code des sondes usdt pour les utiliser.

Voici un [exemple](https://github.com/josefbacik/pyusdt/blob/main/src/benchmark.py) de code Python :
...
Ainsi, elles ont ensuite été améliorées avec la version 3.14 (sortie en 2014) :
...
Les uProbes sont alors devenues pleinement exploitables.
...
{{< alert >}}
Les uRetProbes ont été introduites de façon conjointes aux uProbes.
{{< /alert >}}
...
Pour finir la présentation, je vous partage quelques liens bien sympathiques en rapport avec les uProbes :
...
* [L'excellent article de blog](https://jvns.ca/blog/2017/07/05/linux-tracing-systems/) de **Julia Evans** sur tous les systèmes de tracing sous Linux. Vous pouvez également lire son *zine* :
